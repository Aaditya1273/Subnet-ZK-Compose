"""
FarmOracle Backend API
Africa's Autonomous AI Farming Oracle on the Blockchain
Main FastAPI application for Africa Blockchain Festival 2025
"""
...
app = FastAPI(
    title="FarmOracle API", 
    version="1.0.0",
    description="Africa's Autonomous AI Farming Oracle on the Blockchain - Built for Africa Blockchain Festival 2025"
)
...
def health_check_detailed():
    ...
    status = {
        ...
        "hackathon": "Africa Blockchain Festival 2025",
        "oracles": {
            "soil_oracle": soil_model is not None,
            "disease_oracle": plantdoc_predict_func is not None,
            "market_oracle": price_predict_func is not None
        }
    }
    return status
...
async def startup_event():
    logger.info("\n🌍 FarmOracle API Starting...")
    logger.info("🏆 Built for Africa Blockchain Festival 2025")
    ...
    try:
        logger.info("🔄 Attempting to pre-load AI oracles...")
        
        # Try to load soil oracle
        try:
            load_soil_model()
            logger.info("✅ Soil Oracle loaded successfully")
        except Exception as e:
            logger.warning(f"⚠️ Could not pre-load Soil Oracle: {e}")
        
        # Try to load disease oracle
        try:
            load_plantdoc_predictor()
            logger.info("✅ Disease Oracle loaded successfully")
        except Exception as e:
            logger.warning(f"⚠️ Could not pre-load Disease Oracle: {e}")
        
        # Try to load market oracle
        try:
            load_price_predictor()
            logger.info("✅ Market Oracle loaded successfully")
        except Exception as e:
            logger.warning(f"⚠️ Could not pre-load Market Oracle: {e}")
            
    except Exception as e:
        logger.warning(f"⚠️ Oracle pre-loading failed: {e}")
        logger.info("📝 Oracles will be loaded on first use")
    
    logger.info("✅ FarmOracle API is ready!")
    logger.info("🌍 Empowering African farmers with AI & blockchain!")
